...
const statusStyle: Record<string, string> = {
  Pending: "bg-[#FFF8E1] text-[#F9AB00]",
  Processing: "bg-[#FFF8E1] text-[#F9AB00]",
...
export default function OrdersPage() {
  ...
  return (
    <main className="flex-1 px-1 py-4 bg-transparent min-h-screen">
      ...
      {/* 🔒 FIXED TOP BAR - Topbar (h-20) ke thik niche sticky rahega */}
        ...
        <div className="flex flex-col md:flex-row items-center gap-3">
          <div className="relative w-full md:flex-1">
            ...
            <input
              ...
              value={search}
              // ✅ FIXED: State "search" ko update karne ke liye setSearch use kiya
            ...
            <tbody className="divide-y divide-[#F5F5F5]">
              {filteredOrders.map((o) => (
                <tr key={o.id} className="hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className="py-4 px-6">
                    <div className={`inline-flex px-3 py-1.5 rounded-full text-[10px] font-bold ${statusStyle[o.status] || "bg-gray-100"}`}>
        ...
        {filteredOrders.map((o) => (
          <div key={o.id} className="bg-white rounded-[24px] p-5 border border-[#F0F0F0] shadow-sm shadow-black/5">
            <div className="flex justify-between items-start mb-4">
              ...
              <div className={`px-3 py-1 rounded-full text-[10px] font-bold ${statusStyle[o.status] || "bg-gray-100"}`}>{o.status}</div>
            </div>
